...
class SchemaFront
{
	...
	public $source = 'DataFeed:WordPress';
	...
	*/
                ...
                if ($JSON) {
                    ...
                    if ($RenderSchemaMarkup) {
                        if ($SchemaMarkupType == "App") {
                            ...
                            // Any other markup
                            printf('<script type="application/ld+json" data-source="DataFeed:WordPress" data-schema="%s-%s-%s">%s</script>' . "\n", esc_attr($post->ID), esc_attr($PostType), esc_attr($SchemaMarkupType), wp_json_encode($DecodedMarkup));
                        }
                    }
                }
            ...
            if (!empty($this->Settings['SchemaWebSite']) && is_front_page()) {
                ...
                if (!empty($SchemaMarkupWebSite)) {
                    ...
                    if ($JSON) {
                        ...
                        printf('<script type="application/ld+json" data-source="DataFeed:WordPress" data-schema="Website">%s</script>' . "\n", wp_json_encode($DecodedMarkupWebsite));
                    }
	 ...
	 *
	 * @param array $markup Markup array. This is going to contain the default markup, website, breadcrumb markup.
	 * @return void
	 */
	public function post_to_page_count(array $markup)
	{
		...
		// Format markup for API to include URI, source, and content
		$jsonItems = [];
		foreach ($markup as $item) {
			// Handle case where Custom markup could possibly have no @id
			$atId = isset($item->{'@id'}) ? $item->{'@id'} : "$url#Custom";
			$jsonItems[$atId] = [
