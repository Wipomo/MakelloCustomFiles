...
class Parser
{
    ...
    private function filterBodyNodes(Node $node, bool $nested = false)
    {
        ...
        if ($nested && ($node instanceof BlockReferenceNode || $node instanceof \Twig_SupTwg_Node_BlockReference)) {
        ...
        // we need to discard the wrapping "Twig_SupTwg_Node" for the "body" node
        $nested = $nested || ('Twig_SupTwg_Node' !== \get_class($node) && Node::class !== \get_class($node));
        foreach ($node as $k => $n) {
...
class_alias('Twig\Parser', 'Twig_SupTwg_Parser');
